...
async function enableSensors() {
  ...
  if (motionGranted && orientGranted) {
    ...
    // Start continuous transmission loop (~60 fps / every 16ms for raw fluidity)
    setInterval(sendSensorData, 16);

    // Request Screen Wake Lock to prevent phone sleep
    requestWakeLock();
  } else {
...
async function requestWakeLock() {
  try {
    if ('wakeLock' in navigator) {
      ...
      console.log('Screen Wake Lock activated on phone controller.');
